...
void RCC_voidDisableClock(Bus_t Copy_BusId, u8 Copy_u8BperId) 
{
    if(Copy_u8BperId <= 31)
	{
		switch(Copy_BusId)
		{
			case RCC_AHB  : CLEAR_BIT(RCC_AHBENR,  Copy_u8BperId); break;
			case RCC_APB1 : CLEAR_BIT(RCC_APB1ENR, Copy_u8BperId); break;
			case RCC_APB2 : CLEAR_BIT(RCC_APB2ENR, Copy_u8BperId); break;
			//default       : /*TODO: Return Error Code*/          break;
		}
	}
	else
...
/*      MAIN BUS SYSTEM INIT             */
	...
	//	while(!(CHECK_BIT(RCC_CR, RCC_CR_HSIRDY)));   // Wait Until CLK Ready
        RCC_CFGR |= RCC_CFGR_SW_HSI;
			...
			while(!(CHECK_BIT(RCC_CR, RCC_CR_PLLRDY)));
