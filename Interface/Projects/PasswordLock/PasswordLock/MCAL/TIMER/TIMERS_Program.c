...
ptr_VoidFcn TIMER1_CTCA_Callback ; 
...
void Timer_voidInitTimer1(Timer1Config_t *ptr_userConfig)
{
    ...
    { 
        ...
        TIM_TCCR1A = 0  ; 
		// Slect Mode 
		if(GLOBAL_ptrToTimer1UserConf->operationModeTIM1 == TIM_CTC_Mode)
		{
		  TIM_TCCR1B |= (1<< TCCR1B_WGM12); 
		}
        // Disable Timer Module 
            // by default disable 
		//Set Global Variable 
...
void Timer_voidSetCompareValue(TimerSelection_t copyTimerIndex , u16 copy_u16CompVal)
{
		...
		case TIMER0:
			...
			break;
		case TIMER1:
           if(copy_u16CompVal >=0 && copy_u16CompVal <= 65536)
            {
                TIM_OCR1A = copy_u16CompVal ; 
            }
			break ;
		case TIMER2:
			break;
...
void Timer_voidSetCompareMatchCallback(TimerSelection_t copyTimerIndex , ptr_VoidFcn cmpMatchCallback)
{
	if(cmpMatchCallback != NULL) 
	{
		...
		case TIMER0:
			...
			break;
		case TIMER1:
			TIMER1_CTCA_Callback = cmpMatchCallback ; 
			break ;
		case TIMER2:
			break;
		default:
...
void Timer_voidSetCompMatchInterrupState(TimerSelection_t copyTimerIndex , TIM_OverflowInterrupt copyTimerState)
{
    ...
    case TIMER0:
        ...
        break;
    case TIMER1:
		CLR_BIT(TIM_TIMSK,TIMSK_OCIE1A); 
		// enable compare interrupt
		TIM_TIMSK |= (copyTimerState << TIMSK_OCIE1A); 
	  break;
    case TIMER2:
	    break;
    default:
